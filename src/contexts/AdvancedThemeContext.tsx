...
export const AdvancedThemeProvider: React.FC<AdvancedThemeProviderProps> = ({ children }) => {
  ...
  const applyThemeToDOM = (theme: CustomTheme, darkMode: boolean) => {
    ...
    // Apply dark/light mode class first
    root.classList.remove('light', 'dark');
    ...
    if (!isDefaultTheme) {
      ...
      // Convert HSL string to space-separated values for Tailwind
      ...
      const cssVars = ['--primary', '--secondary', '--accent', '--background', '--foreground', 
  ...
  const toggleDarkMode = async () => {
    ...
    try {
      ...
      // Then apply the full theme with the new mode
      applyThemeToDOM(currentTheme, newDarkMode);
      
      // Add visual feedback for successful toggle
